...
\chapter{汇编语言{---}往年考题}
    ...
    0x0000000000400527 <f+71>:    jmp     0x400513 <f+51>
        ...
        最后确定中间的步骤。\verb|f+29| 处计算了 \verb|n - 1| 到 \verb|%rdi| 中并递归调用，说明返回了 \verb|f(n - 1, m)|。这个值接着在 \verb|f+37| 处和 \verb|%rbx| 存储的 \verb|m| 进行了 \verb|f(n - 1, m) + m - 1| 的算数运算，结果在 \verb|%ebx| 中。注意接下来复制 \verb|%ebx| 到 \verb|%eax| 并把 \verb|%ebx| 算术右移 31 位，相当于填满了 \verb|%ebx| 的符号位。我们知道指令 \verb|idiv| 的行为是这样，以 \verb|%ebx:%eax| 连接作为被除数，操作数作为除数，计算除法，商在 \verb|%eax| 中，余数在 \verb|%ebx| 中（这样的行为和计算机中除法器的实现方法有关，以后会学到）。于是这个填满符号位的操作相当于给 \verb|%eax| 作符号扩展。\verb|idiv| 之后的余数加 1 作为返回值返回了，因此递归主体的两空填写 \verb|int r = f(n - 1, m);|、\verb|return (r + m - 1) % n + 1|。
